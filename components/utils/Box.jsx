...
const Box = ({ headline, desc, project, }) => {
  ...
  return (
    <section className={`flex justify-between items-baseline cursor-pointer ${lower975 ? "w-11/12" : "w-10/12"} ${lower790 ? "w-11/12 " : ""} m-auto text-white ease-in duration-300 ${isHovered ? "shadow-4xl" : ""} p-8 font-Roboto_mono bg-background rounded-xl`} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <div className="text">
      ...
      <div className="icon ">
        <BiCodeAlt className={`text-3xl ease-in duration-300 ${isHovered ? "text-green" : ""} `} />
      </div>
    </section>
  );
};
